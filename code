...
library(quantmod) # Get historical data from yahoo finance, and quantitative financial modeling framework
library(quadprog) # Solve a Quadratic Programming Problem
library(zoo) # Time series and ordered objects
library(tseries) # Time series analysis
library(ggplot2) # Plot graphs 
library(PerformanceAnalytics) #  Risk management and Sensitivity analysis
library(psych)

###############################################################
# Download ETF data set from Yahoo Finance.
# Use library(quantmod) and library(tseries) to download data
# for selected ETF into data environment. In order to ease the 
# impact of stock split and dividends, use adjusted close 
# price of ETFs. Moreover, to test the change of the Market
# environment, use VIX, which is the volatility of
# SPY, to be the indicator of the change of market environment.
###############################################################
# select ETFs
sym_bols <- c("FXE", "EWJ","GLD","QQQ","SPY","SHV","DBA","USO", "XBI","ILF","GAF","EPP","FEZ")

data_env <- new.env()  # new environment for data
# download data for sym_bols into data_env
getSymbols(sym_bols, env=data_env, adjust=TRUE, from="2006-07-12", to="2015-03-01")
